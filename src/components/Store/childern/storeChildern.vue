<template>
    <div id="storeChildern" class="relative" :class="(showImage) ? 'blur bg-black/40 select-none' : ''">
        <div class="w-[95%] md:w-[50%] h-full m-auto my-10">
            ...
            <div
                class="w-full md:w-[80%] h-[400px] flex justify-center items-center mt-5 bg-[#262626] m-auto rounded-xl overflow-hidden border-[10px] border-[#444444] drop-shadow-xl z-50">
                <div class="w-full h-full rounded-xl overflow-hidden">
                    <YoutubeVue3 ref="youtube" :videoid="store.video" width="100%" height="100%" :controls="1" />
                </div>
            </div>
            <div class="image md:w-[70%] bg-[#262626] px-5 py-5 rounded-xl m-auto mt-[-5px]">
                <Carousel :items-to-show="2.5" :wrap-around="true" class="cursor-pointer">
                    <Slide v-for="(slide, idx) in img" :key="idx" class="h-[100px] md:h-[200px]">
            ...
            <div class="w-full my-10 bg-[#262626] rounded-2xl text-start p-10">
                ...
                <div class="flex justify-between items-center mt-5 gap-4">
                    <button @click="AddCart()"
                        class="w-[50%] h-[50px] md:h-[70px] rounded-xl border-2 text-lg hover:bg-white hover:text-black duration-300">Add
                        to
                        Cart</button>
                    <button @click="Purchase"
                        class="text-white w-[50%] h-[50px] md:h-[70px] rounded-xl text-lg bg-[#3d7fa1] hover:bg-[#214f66] duration-300">Purchase</button>
                </div>
            </div>
        </div>
        <ButtonCart ref="cart" />
